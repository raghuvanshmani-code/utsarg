  ...
  const handleFormSubmit = (values: any) => {
      ...
      if (selectedPost) {
          ...
          addDoc(collectionRef, { ...data, createdAt: serverTimestamp() }).then(() => {
              ...
              const permissionError = new FirestorePermissionError({ path: 'blog', operation: 'create', requestResourceData: data });
              errorEmitter.emit('permission-error', permissionError);
          }).finally(() => {
              setIsSubmitting(false);
          });
  ...
  const handleJsonSubmit = async (jsonContent: string) => {
    ...
    try {
        ...
        if (e.message.includes('permission-denied')) {
             const permissionError = new FirestorePermissionError({ path: 'blog', operation: 'create', requestResourceData: JSON.parse(jsonContent) });
             errorEmitter.emit('permission-error', permissionError);
        } else {
